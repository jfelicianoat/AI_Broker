...
**El árbitro se elige por capacidad, no por rapidez (`selection.arbiter_policy`):** ordenar por tiempo esperado es lo correcto para quien tiene que *responder* —cuanto menos espera el usuario, mejor—, pero el árbitro no responde: lee las propuestas y decide cuál es cierta, y ahí lo rápido que escriba no dice nada. Con `strongest_available` (por defecto) los candidatos a árbitro se ordenan por **tamaño declarado** —lo que dice el runtime, y solo si no hay nada, lo que anuncia el nombre—, dejando el orden por tiempo como desempate; un modelo que no cabe en la máquina queda el último aunque sea el mayor, porque elegirlo es elegir un fallo seguro. `fastest_available` conserva el comportamiento anterior. Un `target_model` o `preferred_model` mandan sobre la política.

El tamaño es un indicio pobre de capacidad y es el único que hay: el histórico mide tiempo y tasa de fallo, no acierto. **Ojo al alcance:** «el más capaz disponible» incluye los modelos cloud si la tarea los permite, así que en un catálogo con API el árbitro puede dejar de ser local (y de ser gratis). La frontera de datos sigue mandando: una tarea confidencial nunca sale de la máquina.

**Cuando un modelo deja de servir (cuarentena):** un modelo puede cargar sin protestar y aun así no responder —pesos corruptos, una cuantización que el runtime ejecuta mal, una plantilla rota—. Dos fallos **definitivos** seguidos, sin ningún acierto entre medias, lo sacan de la selección 24 h; la cuarentena **caduca sola**, así que un modelo reinstalado vuelve sin rehabilitarlo a mano. Quedarse sin VRAM o un proveedor caído son circunstancias y **a propósito no cuentan**: castigarían a los modelos grandes sanos.

Para que eso funcione hay que **mirar la salida**, no solo si hubo respuesta. Un modelo puede devolver un cuerpo perfectamente formado que no es una respuesta, y entonces nada falla de forma visible: se guarda como éxito y se entrega. El broker rechaza dos formas de esto y las cuenta como fallo del modelo:

- `PROMPT_ECHOED` — devuelve su propio prompt en lugar de contestar (≥60% de la salida es copia literal larga de la entrada).
- `DEGENERATE_OUTPUT` — se queda repitiendo un ciclo corto hasta el final de la respuesta. Se mira solo la **cola**: si el modelo sale del bucle y termina de responder, su respuesta sirve. Calibrado contra 18.319 salidas reales del histórico sin un solo falso positivo.

La salida rechazada se guarda en la invocación con su coste real, que es la única forma de auditar después qué devolvió.
...
| `/api/v1/tasks/{id}/artifacts` | GET | Ficheros que produjo la tarea (imágenes generadas, salida en disco) |
| `/api/v1/tasks/{id}/artifacts/{artifact_id}` | GET | Descarga de un artefacto |
