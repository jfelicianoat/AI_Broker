...
- El token admin **se genera nuevo en cada arranque** (salvo que `AI_BROKER_ADMIN_TOKEN` venga definida desde fuera, que es la vía para fijarlo estable) y se imprime en la consola del broker. Si en esa misma máquina corre un cliente que arranca solo —un runner lanzado por Wake-on-LAN o por una tarea programada—, esa consola no la ve nadie: activa `server.publish_session_token: keyring` y el broker dejará el token de cada sesión en el Administrador de credenciales bajo `ai-broker` / `session_admin_token`, que es de donde debe leerlo. Nunca lo mandes por la red a otra máquina. Detalle en `docs/Configuration_Reference.md` (§`server`) y contrato de cliente en `docs/Client_API.md` §3.1.
