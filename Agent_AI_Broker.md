...
| `/api/v1/tasks/{id}/artifacts/{artifact_id}` | GET | Descarga de un artefacto (`410` si la retención se lo llevó) |
...
- La admisión local tiene además un techo por **conteo** (`resources.max_loaded_local_models`, `auto` = la capacidad paralela de inferencia): la memoria sola no basta en una máquina holgada, donde cuatro modelos medianos caben y se pelean por el mismo bus. Antes de hacer esperar a nadie se descargan los modelos cargados sin lease; solo si los que ocupan el cupo están sirviendo tareas se cede el turno (`LOCAL_MODEL_SLOTS_BUSY`, motivo `model_slots`), igual que ante la falta de memoria. Un modelo ya cargado nunca paga el cupo.
